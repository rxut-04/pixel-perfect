  ...
  return (
    <div onMouseEnter={() => setActive(item)} className="relative">
      <motion.p
        ...
        className="cursor-pointer text-foreground hover:opacity-90 font-medium"
      >
        {item}
      </motion.p>
      {active !== null && (
...
export const Menu = ({
  ...
  return (
    <nav
      ...
      className="relative rounded-full border border-border bg-card shadow-smooth flex justify-center space-x-4 px-8 py-6"
    >
      {children}
    </nav>
  );
...
export const ProductItem = ({
  ...
  return (
    <Link to={href} className="flex space-x-2 group">
      ...
      />
      <div>
        <h4 className="text-xl font-bold mb-1 text-foreground group-hover:text-accent transition-colors">
          {title}
        </h4>
